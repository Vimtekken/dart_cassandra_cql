...
main({bool enableLogger: false}) {
  ...
  group("Single query:", () {
    test("invalid binding type exception", () {
      expect(
          () => new cql.Query("SELECT foo FROM bar WHERE baz=:\$ AND boo =:boo",
              bindings: 'boo'),
          throwsArgumentError);
    });
    ...
    test("invalid placeholder exception for named2positionalQuery", () {
      expect(
          () => new cql.Query("SELECT foo FROM bar WHERE baz=:\$ AND boo =:boo",
              bindings: ['boo']).positionalQuery,
          throwsArgumentError);
    });

    test("missing custom type codec exception", () {
      custom.CustomJson customJson = new custom.CustomJson({"foo": "bar"});
      ...
      expect(
          ...
          throwsA((e) =>
              e is ArgumentError &&
              e.message ==
                  "No custom type codec specified for type with class: ${customJson.customTypeClass}"));
    });

    test("named to positional query missing bindings exception", () {
      cql.Query query = new cql.Query(
          "SELECT foo FROM bar WHERE baz=:baz AND boo=:boo AND baz2=:baz");
      expect(query, new isInstanceOf<cql.QueryInterface>());
      query.positionalQuery;
      query.bindings = {'baz': true};
      expect(
          () => query.namedToPositionalBindings,
          throwsA((e) =>
              e is ArgumentError &&
              e.message == "Missing binding for named placeholder 'boo'"));
    });
  });
  ...
  group("Single query expansion", () {
    ...
    test("with empty bindings", () {
      expect(
          new cql.Query("SELECT foo FROM bar WHERE baz='Simple string' LIMIT 1")
              .expandedQuery,
    ...
    group("of positional args:", () {
      test("ASCII/TEXT", () {
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=? LIMIT 1",
                bindings: ['Simple string']).expandedQuery,
            "SELECT foo FROM bar WHERE baz='Simple string' LIMIT 1");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?",
                bindings: ['Simple string']).expandedQuery,
            "SELECT foo FROM bar WHERE baz='Simple string'");
        expect(
            new cql.Query(
                    "SELECT foo FROM bar WHERE skip='this?' AND baz=? OR boo=?",
                    bindings: [r"'Quoted' string", r"Yet \''Another'\' string"])
                .expandedQuery,
            r"SELECT foo FROM bar WHERE skip='this?' AND baz='''Quoted'' string' OR boo='Yet \''''Another''\'' string'");
      });
      test("INT/BIGINT/COUNTER", () {
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?", bindings: [1])
                .expandedQuery,
            "SELECT foo FROM bar WHERE baz=1");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?", bindings: [-3812])
                .expandedQuery,
            "SELECT foo FROM bar WHERE baz=-3812");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?",
                bindings: [9223372036854775807]).expandedQuery,
            "SELECT foo FROM bar WHERE baz=9223372036854775807");
      });
      test("BLOB", () {
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?", bindings: [
              new Uint8List.fromList([0xFE, 0xed, 0xfA, 0xCe])
            ]).expandedQuery,
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?", bindings: [
              new Uint8List.fromList(
                  [0x8B, 0xAD, 0xF0, 0x0D, 0xF0, 0x0C, 0x0F, 0xFE])
            ]).expandedQuery,
      ...
      test("SET", () {
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?", bindings: [set1])
                .expandedQuery,
            "SELECT foo FROM bar WHERE baz=['abc','fe''f']");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?", bindings: [set2])
                .expandedQuery,
      ...
      test("TUPLE", () {
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?", bindings: [tuple])
                .expandedQuery,
            "SELECT foo FROM bar WHERE baz=('abc','fe''f')");
      });
      test("UDT", () {
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=?", bindings: [udt])
                .expandedQuery,
    ...
    group("of named args:", () {
      test("ASCII/TEXT", () {
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz LIMIT 1",
                bindings: {'baz': 'Simple string'}).expandedQuery,
            "SELECT foo FROM bar WHERE baz='Simple string' LIMIT 1");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': 'Simple string'}).expandedQuery,
            "SELECT foo FROM bar WHERE baz='Simple string'");
        expect(
            new cql.Query(
      ...
      test("INT/BIGINT/COUNTER", () {
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': 1}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=1");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': -3812}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=-3812");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': 9223372036854775807}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=9223372036854775807");
      });
      test("CUSTOM", () {
        custom.CustomJson customJson = new custom.CustomJson({"foo": "bar"});
        ...
        cql.registerCodec(
            customJson.customTypeClass, new custom.CustomJsonCodec());
      ...
      test("BOOLEAN", () {
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz AND boo=:boo",
                bindings: {'baz': true, 'boo': false}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=true AND boo=false");
      });
      test("DECIMAL/FLOAT/DOUBLE", () {
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': 12.31415161718}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=12.31415161718");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': -12314151617.18}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=-12314151617.18");
      });
      test("TIMESTAMP", () {
        DateTime now = new DateTime.now();
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=   :test",
                bindings: {'test': now}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=   ${now.millisecondsSinceEpoch}");
      });
      test("INET", () {
        InternetAddress ipv4 = new InternetAddress("192.168.169.101");
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz AND boo=:boo",
                bindings: {'boo': ipv6, 'baz': ipv4}).expandedQuery,
            "SELECT foo FROM bar WHERE baz='${ipv4.address}' AND boo='${ipv6.address}'");
      });
      test("UUID", () {
        cql.Uuid uuid = new cql.Uuid.simple();
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:uuid",
                bindings: {'uuid': uuid}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=${uuid.value}");
      });
      test("LIST", () {
        expect(
      ...
      test("SET", () {
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': set1}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=['abc','fe''f']");
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': set2}).expandedQuery,
            r"SELECT foo FROM bar WHERE baz=[1,2]");
      });
      test("TUPLE", () {
        cql.Tuple tuple = new cql.Tuple.fromIterable(['abc', "fe'f"]);
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': tuple}).expandedQuery,
            "SELECT foo FROM bar WHERE baz=('abc','fe''f')");
      });
      test("UDT", () {
        Map udt = {
        ...
        expect(
            new cql.Query("SELECT foo FROM bar WHERE baz=:baz",
                bindings: {'baz': udt}).expandedQuery,
            "SELECT foo FROM bar WHERE baz={'a': 1, 'b': 'fe''f', 'c': [{'foo': 'bar'},{'foo': 'baz'}]}");
      });
  ...
  group("Batch query expansion", () {
    ...
    test("mixed bindings", () {
      cql.BatchQuery batch = new cql.BatchQuery()
        ..add(new cql.Query(
            "SELECT foo FROM bar WHERE baz='Simple string' LIMIT 1"))
        ..add(new cql.Query("SELECT foo FROM bar WHERE baz=? LIMIT 1",
            bindings: ['Simple string']))
        ..add(new cql.Query("SELECT foo FROM bar WHERE baz=:baz LIMIT 1",
            bindings: {'baz': 'Simple string'}));
      expect(batch, new isInstanceOf<cql.QueryInterface>());
      expect(batch.queryList.map((cql.Query q) => q.expandedQuery), [
        "SELECT foo FROM bar WHERE baz='Simple string' LIMIT 1",
        "SELECT foo FROM bar WHERE baz='Simple string' LIMIT 1",
        "SELECT foo FROM bar WHERE baz='Simple string' LIMIT 1"
      ]);
  ...
  group("V2, V3 compatibility:", () {
    test("named to positional query", () {
      cql.Query query = new cql.Query(
          "SELECT foo FROM bar WHERE baz=:baz AND boo=:boo AND baz2=:baz");
      expect(query.positionalQuery,
          "SELECT foo FROM bar WHERE baz=? AND boo=? AND baz2=?");
    });
    test("positional to positional query", () {
      cql.Query query =
          new cql.Query("SELECT foo FROM bar WHERE baz=? AND boo=? AND baz2=?");
      expect(query.positionalQuery,
          "SELECT foo FROM bar WHERE baz=? AND boo=? AND baz2=?");
    });
    test("named to positional bindings", () {
      cql.Query query = new cql.Query(
          "SELECT foo FROM bar WHERE baz=:baz AND boo=:boo AND baz2=:baz");
      query.positionalQuery;
      query.bindings = {'baz': true, 'boo': false};
      expect(query.namedToPositionalBindings, [true, false, true]);
    });
